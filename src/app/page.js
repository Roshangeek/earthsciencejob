  ...
  const allExams = [
    {
      ...
      status: "Admit Card Released / Notification Live",
      examDate: "June 2026",
      eligibility: "M.Sc in Geology/Applied Geology/Marine Geology/Hydrogeology or M.Sc Geophysics",
      url: "https://upsconline.nic.in"
    },
    {
      id: 2,
      title: "GATE 2026 Geology & Geophysics (GG) / Engineering Sciences (XE)",
      conductingBody: "IIT Roorkee / IISc Bangalore",
      status: "Scorecard & PSU Recruitment Active",
      examDate: "February 2026",
      eligibility: "Bachelor's/Master's in relevant science or engineering branches",
      url: "https://gate2026.iitr.ac.in"
    },
    {
      id: 3,
      title: "CSIR UGC NET Earth, Atmospheric, Ocean and Planetary Sciences",
      conductingBody: "National Testing Agency (NTA / CSIR)",
      status: "Application Window Open",
      examDate: "July 2026",
      eligibility: "M.Sc or equivalent degree with 55% marks",
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8 pb-6 border-b border-slate-800">
        <div>
          ...
          <h1 className="text-xl font-bold tracking-tight text-white">Geoscience Careers, Exams & Grants</h1>
        </div>
        <div className="flex items-center gap-2 bg-slate-900 p-1.5 rounded-xl border border-slate-800 flex-wrap">
          <Link href="/?tab=jobs" className={`px-3 py-1.5 rounded-lg text-xs font-bold transition ${activeTab === 'jobs' ? 'bg-teal-600 text-white shadow' : 'text-slate-400 hover:text-white'}`}>
            💼 Jobs
          </Link>
          <Link href="/?tab=exams" className={`px-3 py-1.5 rounded-lg text-xs font-bold transition ${activeTab === 'exams' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-white'}`}>
            📝 Exams & Admit Cards
          </Link>
          <Link href="/?tab=scholarships" className={`px-3 py-1.5 rounded-lg text-xs font-bold transition ${activeTab === 'scholarships' ? 'bg-amber-600 text-white shadow' : 'text-slate-400 hover:text-white'}`}>
            🎓 Scholarships
          </Link>
          <Link href="/admin" className="px-3 py-1.5 rounded-lg text-xs font-bold text-slate-300 bg-slate-800 hover:bg-slate-700 transition">
            ⚙️ Admin
          </Link>
        </div>
      </div>
      ...
      {activeTab === 'jobs' ? (
        ...
        <>
          <div className="text-center max-w-2xl mx-auto mb-8">
            <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight mb-2 text-white">
              Competitive Earth Science Examinations & Admit Cards
            </h2>
            <p className="text-slate-400 text-xs">
              Track official notifications, syllabus updates, exam dates, and admit cards for UPSC, GATE, and CSIR NET.
            </p>
          </div>

          <div className="space-y-4 max-w-3xl mx-auto">
            {allExams.map((exam) => (
              <div key={exam.id} className="bg-slate-900/80 p-6 rounded-2xl border border-slate-800/80 shadow-lg">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
                  <div>
                    <span className="text-[10px] font-bold uppercase bg-indigo-950 text-indigo-300 px-2.5 py-1 rounded border border-indigo-800/60 font-mono">📝 National Exam Notice</span>
                    <h3 className="text-base font-bold text-white mt-2">{exam.title}</h3>
                    <p className="text-xs text-teal-400 font-medium">Conducting Authority: {exam.conductingBody}</p>
                  </div>
                  <a href={exam.url} target="_blank" rel="noopener noreferrer" className="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider text-center w-full md:w-auto shadow transition">
                    Official Notice / Portal ↗
                  </a>
                </div>
                ...
                <div className="bg-slate-950/90 rounded-xl p-3.5 border border-slate-800 text-xs space-y-2 text-slate-300 font-mono">
                  <div className="flex justify-between border-b border-slate-900 pb-1.5">
                    ...
                    <span className="font-bold text-emerald-400">{exam.status}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-900 pb-1.5">
                    <span className="text-slate-400">Eligibility Criteria:</span>
                    <span className="font-semibold text-slate-200 text-right max-w-md">{exam.eligibility}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Exam Window:</span>
                    <span className="font-semibold text-amber-400">{exam.examDate}</span>
                  </div>
                </div>
              </div>
            ))}
